return function()
	...
	__rt.spawn(function()
		do
			...
			local timer2 = __rt.timer_start(start + 10000000, 0, 0)
			local timer3 = __rt.timer_start(start + 10000000, 0, 0)
			local timer4 = __rt.timer_start(start + 10000000, 0, 0)
			local timer5 = __rt.timer_start(start + 10000000, 0, 0)
			local timer6 = __rt.timer_start(start + 10000000, 0, 0)
			print("timer_start =>", timer)
			print("timer_stop =>", __rt.timer_stop(timer))
			-- collectgarbage("collect")
			-- local what, sender, data = __rt.recv()
			-- print("recv =>", what, sender, data, ((__rt.time() - start) / 1000000).."ms")
		end
		-- collectgarbage("collect")
	end)
	-- update the timer to expire 500ms later than we initially asked for
	__rt.timer_update(timer1, wait_until + 500000000, 0, 0)
