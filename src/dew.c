...
#else
	static void setup_require_path(lua_State* L, const char* script_file) {
		char file[PATH_MAX+1];
		// realpath(script_file, file);
		snprintf(file, sizeof(file), "%s", script_file);
...
static int pmain(lua_State* L) {
	...
	// load dew Lua script
		...
		if (!filename || *filename == 0)
			filename = "src/dew.lua";
		setup_require_path(L, filename);
